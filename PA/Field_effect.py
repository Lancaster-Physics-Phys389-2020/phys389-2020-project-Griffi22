...
#                         print('particle angle',Phi_p,'E before angle',Phi_E_i0,'E after angle',Phi_E_i1)
                         ...
                         E_pot_index=sign*(self.E_prop_list[1][Index]/(2*scipy.constants.epsilon_0)*(((np.linalg.norm(P_to_Es_Distance)**2+self.E_prop_list[2]**2)**0.5)-np.linalg.norm(P_to_Es_Distance)))
                         E += E_index
                         E_pot += E_pot_index
#                     print(i)
#                     print('index',   i)# self.E_plate_index)
                     self.E_plate_index=i%(len(self.E_prop_list[0])-2)
                     return(E,self.E_plate_index,E_pot)
